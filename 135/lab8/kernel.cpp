...
int edgeDetectValue(int a, int b, int c, int d, int e, int f, int g, int h, int i) {
  return (g+2*h+i)-(a+2*b+c);
}

void edgeDetect(int original[MAX_H][MAX_W], int output[MAX_H][MAX_W], int height, int width) {
  for (int row = 0; row < height; row++) {
    for (int col = 0; col < width ; col++) {
      int a, b, c, d, e, f, g, h, i = 0;
      //For pixels on the very edge, we count less neighbor pixels.
      //if row or col is 0 or equal to height or width,
      //ignore either the top, bottom, or side faces
      //if both is true, ignore two faces.
      //e corresponds to original[row][col]
			if (row != 0) {
				a = original[row-1][col-1];
	      b = original[row][col-1];
	      c = original[row+1][col-1];
			}
			if (row != height) {
				g = original[row-1][col+1];
	      h = original[row][col+1];
	      i = original[row+1][col+1];
			}
			if (col != 0) {
				a = original[row-1][col-1];
				d = original[row-1][col];
				g = original[row-1][col+1];
			}
			if (col != width) {
				c = original[row+1][col-1];
				f = original[row+1][col];
				i = original[row+1][col+1];
			}
      e = original[row][col];

      int detected = edgeDetectValue(a, b, c, d, e, f, g, h, i);
      output[row][col] = detected;
    }
  }
}
